...
>> Algorithm Structure for MeetingTime problem instance:
Create Data Structures
Find & Parse File(Data Structures)
Identify & Output(Data Structures) # can be split in two if output data is saved

I/O:
I - A specifically formatted txt file with two person's schedules written in military time without conflicts and a specified minimum meeting time
O - A txt file listing the available times between both persons with a [[XX:XX], [YY:YY]] format

The main file above should be written with the functions written in a different file for readability.
'''

def main():
    # DEBUGGER_TOOL.createSampleSchedule()

    fileName = "schedule.txt"
    workerMeetingTimes = []
    workerScheduledHours = [] # Example: 9AM-5PM, written as [09:00,17:00]
    minimumMeetingTime = -1

    # Loop 1440 times with bool True
    availableTimes = []
    _fillBoolAvailableTime(availableTimes)

    minimumMeetingTime = MT_READ_FILE.parseFile(workerMeetingTimes, workerScheduledHours, fileName)
    flagUnavailableTimes(availableTimes, workerMeetingTimes, workerScheduledHours)
    scanValidMeetingTimes(availableTimes, minimumMeetingTime)
...
def scanValidMeetingTimes(availableTimes, minimumMeetingTime):
